...
class DataCollatorForT5MLM:
    ...
    def __call__(self, examples: List[Union[List[int], Any, Dict[str, Any]]]):
        ...
        input_ids_mask = torch.stack([self.random_spans_noise_mask(expandend_input_length) for _ in range(batch_size)])
        labels_mask = ~input_ids_mask

        input_ids_sentinel = self.create_sentinel_ids(input_ids_mask.to(torch.int8))
    ...
    def random_spans_noise_mask(self, length):
        ...
        num_noise_tokens = int(torch.round(torch.tensor(length * self.noise_density)))
        # avoid degeneracy by ensuring positive numbers of noise and nonnoise tokens.
        num_noise_tokens = min(max(num_noise_tokens, 1), length - 1)
        num_noise_spans = int(torch.round(torch.tensor(num_noise_tokens / self.mean_noise_span_length)))
...
def main():
    ...
    def tokenize_function(examples):
        with CaptureLogger(tok_logger) as cl:
            output = tokenizer(examples[text_column_name], return_attention_mask=False)
    ...
    trainer = Trainer(
        ...
        # compute_metrics=compute_perplexity if training_args.do_eval and not is_torch_tpu_available() else None,
